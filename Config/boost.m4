...
# * order changed of search directories for libraries - /usr/lib is now first
# * boost_system always linked with boost_filesystem for newer boost versions
# * option to allow specifying name (e.g. boost_python3) for a Boost library 
# * order changed when searching for Boost library names, to speed up search
# * name of Python configuration file altered for Python 3 naming conventions
# * search for a Boost library omits system paths when user provides location
# * search for a Boost library checks version before testing compatibility
# * test added for the Python suffix "-pyXY" in the name of a Boost library
# * Search for a Boost library includes multiarch names in a search path
# * flag added for GCC C++ preprocessor to fix Boost version search
...
#
# CHANGED FOR GENGEO:
# * A CPP option (-P) has been added to suppress linemarkers from the C++
#   preprocessor output. (In GCC 5.4 a linemarker generated by the 
#   preprocessor splits a line of output used by another macro to identify
#   the Boost version, resulting in configuration failure.)
test $({ echo ${CPPFLAGS} | sed -ne 's;.*\(-P\).*;\1;p'; })x = -Px || CPPFLAGS="-P ${CPPFLAGS}"
...
if test x"$boost_cv_inc_path" != xno; then
  ...
  #
  # CHANGED FOR ESYS-PARTICLE:
  # * boost_cv_lib_version and boost_major_version are now derived from the
  #   BOOST_VERSION C++ definition, as is a new variable, boost_full_version
  #
...
#boost-lib-version = BOOST_LIB_VERSION],
  ...
  AC_CACHE_CHECK([for Boost's header version],
    [boost_cv_full_lib_version],
    [m4_pattern_allow([^BOOST_VERSION$])dnl
     _BOOST_SED_CPP([/^boost-full-lib-version = /{s///p;q;}],
                    [#include <boost/version.hpp>
boost-full-lib-version = BOOST_VERSION],
    [boost_cv_full_lib_version=`cat conftest.i`])])
  let major_version=${boost_cv_full_lib_version}/100000
  let minor_version=${boost_cv_full_lib_version}/100%1000
  let patch_level=${boost_cv_full_lib_version}%100
  #echo Boost Major Version is ${major_version}
  #echo Boost Minor Version is ${minor_version}
  #echo Boost Patch Level is ${patch_level}
  boost_cv_lib_version=${major_version}_${minor_version}
  test "${patch_level}" -ne 0 && boost_cv_lib_version=${boost_cv_lib_version}_${patch_level}
  # e.g. "134" for 1_34_1 or "135" for 1_35
  #boost_major_version=`echo "$boost_cv_lib_version" | sed 's/_//;s/_.*//'`
  boost_major_version=${major_version}${minor_version}
  boost_full_version=`echo ${boost_cv_lib_version} | sed 's/_/\./g'`
  test "${patch_level}" -eq 0 && boost_full_version=${boost_full_version}.0
  echo Boost Version is ${boost_full_version}
...
[Boost_lib=no
  ...
  # CHANGED FOR ESYS-PARTICLE:
  # * Check for $shrext_cmds which stores the shared-object library extension.
  test -z "${shrext_cmds}" &&
    AC_MSG_ERROR([The shrext_cmds variable is empty.  Did you invoke Libtool?])
...
dnl start the for loops).
  ...
  boost_failed_libs=
# Don't bother to indent the following nested for loops, only the 2
# innermost ones matter.
#
# CHANGED FOR ESYS-PARTICLE:
# * Search order changed to give priority first to a library name provided
#   on the command line, second to a library name without suffixes,
#   third to a library name with the -mt suffix before searching for
#   libraries containing other combinations of suffixes in their names
# * Test added for the Python suffix "-pyXY" in the name of a Boost Python library
for boost_lib_ in $2; do
python_suffix=
library_name=${boost_lib_:0:6}
test "${library_name}" = python && python_suffix=-py${PYTHON_MAJOR_VERSION}${PYTHON_MINOR_VERSION}
for boost_tag_ in '' -$boost_cv_lib_tag; do
for boost_ver_ in '' -$boost_cv_lib_version; do
for boost_rtopt_ in '' $boost_rtopt -d; do
for boost_mt_ in '' $boost_mt -mt; do
for boost_py_ in '' ${python_suffix}; do
  for boost_lib in \
    boost_$boost_lib_$boost_tag_$boost_mt_$boost_rtopt_$boost_ver_$boost_py_ \
    boost_$boost_lib_$boost_tag_$boost_rtopt_$boost_ver_$boost_py_ \
    boost_$boost_lib_$boost_tag_$boost_mt_$boost_ver_$boost_py_ \
    boost_$boost_lib_$boost_tag_$boost_ver_$boost_py_
  do
    ...
    #
    # CHANGED FOR ESYS-PARTICLE:
    # * If --with-boost has been set to a folder, assume that the user knows where
    #   the Boost libraries are; do not search standard locations.  This is to
    #   speed up the library search and to help avoid finding libraries that are
    #   incompatible with the headers.  If the option has not been set, add the
    #   standard possible library locations to the path list.
    # * Search order of standard locations now puts /usr/lib* before /usr/local/lib*
    # * Search for a Boost library includes multiarch names in a search path
    if test x"${with_boost}" != x && test x"${with_boost}" != xyes; then
    ...
    for boost_ldpath in ${possible_paths} ''
    do
      # Don't waste time with directories that don't exist.
      (test x"$boost_ldpath" != x && test ! -d "$boost_ldpath") && continue
      boost_save_LIBS=$LIBS
      boost_save_LDFLAGS=$LDFLAGS
      # Are we looking for a static library?
      #
      # CHANGED FOR ESYS-PARTICLE:
      # * To avoid finding shared libraries that are incompatible with the supplied
      #   headers, resulting in a failed build, check first for the existence of a
      #   library that has Boost's full header version suffixed to the usual library
      #   extension.  (This works because Boost and other developers use the 
      #   Libtool versioning system to specify release information, which is the same 
      #   for all libraries.  If Boost in future begins to use the Libtool version 
      #   numbers for their intended purpose, the library tests below will need to 
      #   be revised.)  Allow a library without the version extension only if a 
      #   Boost library name has been specified in the configuration options.  Assume 
      #   in this second case that the user knows the correct library's exact name.  
      #   This change is needed because there have been two situations in which the 
      #   ``configure'' script incorrectly reports finding a Boost library.  The 
      #   first arises in connection with Boost Filesystem: As an example, after 
      #   rejecting the first variant name for Boost System (boost_system) for 
      #   incompatibility and settling on the second (boost_system-mt), the test 
      #   for Boost Filesystem accepts the first variant name (boost_filesystem) rather 
      #   than the second (boost_filesystem-mt) and the subsequent build fails.
      #   (If one forces the Boost Filesystem search to ignore anything discovered
      #   during the Boost System search, provided through LIBS and LDFLAGS, the test
      #   correctly rejects boost_filesystem and settles on boost_filesystem-mt.  But
      #   this approach works not on all systems.)  The second situation arises because
      #   the current way of testing for compatible libraries is to test merely for the 
      #   existence of a library with a matching name and optionally for a function
      #   in the library that is reported by the headers.  Although the function test
      #   successfully screens out libraries that are old enough, it is insufficient
      #   when multiple sets of libraries with closely numbered versions are installed.
      #   Thus although the test will report that version 1.36.0 Boost System libraries 
      #   are incompatible with version 1.55.0 headers, it does not screen out 1.49.0
      #   libraries that are incompatible with 1.52.0 headers, resulting in the
      #   subsequent build failing because of some other function or variable that
      #   was not tested by ``configure''.
      case $boost_ldpath:$boost_rtopt_ in #(
        ...
        (*) # No: use -lboost_foo to find the shared library.
          ...
          link_one=
          link_two=
          echo ${boost_ldpath}/lib${boost_lib}${shrext_cmds} ...
          if test -e "${boost_ldpath}/lib${boost_lib}${shrext_cmds}"
          then
            # Test if the linked file has the correct version in its name.
            if test -L "${boost_ldpath}/lib${boost_lib}${shrext_cmds}" &&
               link_one=$(ls -l ${boost_ldpath}/lib${boost_lib}${shrext_cmds}) &&
               test "${link_one#*${shrext_cmds}.}" = "${boost_full_version}"
            then
              echo lib${boost_lib}${shrext_cmds} links to ${link_one##* }
              echo yes...
            # Occasionally the first linked file is itself a link.  Test if the
            # second linked file has the correct version in its name.
            elif test -L "${boost_ldpath}/lib${boost_lib}${shrext_cmds}" &&
                 test -L "${boost_ldpath}/${link_one##* }" &&
                 link_two=$(ls -l ${boost_ldpath}/${link_one##* }) &&
                 test "${link_two#*${shrext_cmds}.}" = "${boost_full_version}"
            then
              echo lib${boost_lib}${shrext_cmds} links to ${link_one##* }
              echo ${link_one##* } links to ${link_two##* }
              echo yes...
            # If a library name has been provided to ``configure'', assume that the
            # user knows best.
            elif (test -n "${boost_system_library}" &&
                  test "${boost_lib_}" = "${boost_system_library}") ||
                 (test -n "${boost_filesystem_library}" &&
                  test "${boost_lib_}" = "${boost_filesystem_library}") ||
                 (test -n "${boost_python_library}" &&
                  test "${boost_lib_}" = "${boost_python_library}")
            then
              echo "${boost_ldpath}/lib${boost_lib}${shrext_cmds}.${boost_full_version}"\
                   "does not exist, but user-provided library name matches another library"\
                   "in ${boost_ldpath}/ ..."
            else
              echo no...
...
dnl generated only once above (before we start the for loops).
      ...
      if test x"$Boost_lib" = xyes; then
        ...
        # CHANGED FOR ESYS-PARTICLE:
        # * Helpful information for the user in the event that a candidate library
        #   be rejected
        echo "no..."
        echo "There is a problem with locating the correct library. "\
             "The candidate failed a compatibility or linking test. "\
             "The following may apply:"
        if test "${library_name}" = python
        then
          echo "* The discovered Python executable is not the same version as that"\
               "used to build the Boost Python library.  If the remaining search"\
               "fails for this library, either:"
          echo "  - Add PYTHON=pythonX.Y[[d]][[m]][[u]] (X: major version number; Y: minor"\
                 "version number) to the configure command to force a search for the"\
                 "version of Python that matches the discovered Boost Python; or"
          echo "  - If multiple Boost Python libraries corresponding to different"\
                 "Python versions are in the same directory and the discovered file is"\
                 "a link to an incompatible version, then add as an option to the"\
                 "configure command the name of the Boost Python library that is compatible"\
                 "with the discovered version of Python (i.e., --with-boost-python=boost_python-pyXY)."
        fi
        echo "* The version of the candidate library mismatches the version of the"\
             "discovered headers.  If you have added a library name as an option to"\
             "the configure command, check that the name is correct and that the"\
             "version of this library matches the version of the headers."
        echo "* The test program cannot be linked.  See below and config.log for details."
      fi
    done
    # CHANGED FOR ESYS-PARTICLE:
    # * Update for failed boost_lib search is shifted to outside the loop, where it is
    #   updated once after all the possible folders have been checked for its existence.
    #
    # Search for library in all paths exhausted, so ...
...
#
# CHANGED FOR ESYS-PARTICLE:
# * addition of --with-boost-filesystem help-string macro, taken from
#   ax_boost_filesystem.m4, whose license is:
#
#   LICENSE
#
...
#
#   The --with-boost-system and --with-boost-python help-string macros below are
#   derived from this macro.
#
# * Boost.System LIBS and LDFLAGS removed from Boost.Filesystem LIBS and LDFLAGS.
#   This was preventing a proper search for Boost.Filesystem.  A successful
#   search was being reported when no search was being done.  This situation can
#   occur when a system-installed Boost library exists in addition to a
#   user-installed Boost library.
# * replacement of BOOST_FIND_LIB with BOOST_FIND_LIBS
# * addition of $boost_filesystem_library to second parameter of
#   BOOST_FIND_LIBS to allow the macro to be used
...
AC_ARG_WITH([boost-filesystem],
  AS_HELP_STRING([--with-boost-filesystem@<:@=special-lib@:>@],
                 [specify the Boost Filesystem library to use;
    ...
    if test "$withval" = "no"; then
      ...
      want_boost="yes"
      boost_filesystem_library="${withval#boost_}"
    fi
    ],
    [want_boost="yes"])
boost_filesystem_save_LIBS=$LIBS
...
LIBS="$LIBS $BOOST_SYSTEM_LIBS"
LDFLAGS="$LDFLAGS $BOOST_SYSTEM_LDFLAGS"
BOOST_FIND_LIBS([filesystem], [$boost_filesystem_library filesystem], [$1],
...
#
# CHANGED FOR ESYS-PARTICLE:
# * Name of Python configuration file altered to reflect changes in
#   Python 3 to naming conventions
m4_define([_BOOST_PYTHON_CONFIG],
[AC_SUBST([BOOST_PYTHON_$1],
          [`python${PYTHON_VERSION}${PYTHON_VERSION_SUFFIX}-config --$2 2>/dev/null`])dnl
...
#
# CHANGED FOR ESYS-PARTICLE:
# * addition of --with-boost-python help-string macro, modified from
#   --with-boost-filesystem help-string macro above
# * addition of $boost_python_library to second parameter of
#   BOOST_FIND_LIBS to allow the macro to be used
BOOST_DEFUN([Python],
[
AC_ARG_WITH([boost-python],
  AS_HELP_STRING([--with-boost-python@<:@=special-lib@:>@],
                 [specify the Boost Python library to use;
    ...
    if test "$withval" = "no"; then
      ...
      want_boost="yes"
      boost_python_library="${withval#boost_}"
    fi
    ],
    [want_boost="yes"])
...
BOOST_FIND_LIBS([python], [$boost_python_library python python3], [$1],
                [boost/python.hpp],
...
#
# CHANGED FOR ESYS-PARTICLE:
# * addition of --with-boost-system help-string macro, modified from
#   --with-boost-filesystem help-string macro above
# * replacement of BOOST_FIND_LIB with BOOST_FIND_LIBS
# * addition of $boost_system_library to second parameter of 
#   BOOST_FIND_LIBS to allow the macro to be used
BOOST_DEFUN([System],
[
AC_ARG_WITH([boost-system],
  AS_HELP_STRING([--with-boost-system@<:@=special-lib@:>@],
                 [specify the Boost System library to use (Boost 1.35 and above);
    ...
    if test "$withval" = "no"; then
      ...
      want_boost="yes"
      boost_system_library="${withval#boost_}"
    fi
    ],
    [want_boost="yes"])
BOOST_FIND_LIBS([system], [$boost_system_library system], [$1],
